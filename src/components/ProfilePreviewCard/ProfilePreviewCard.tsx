    ...
    if(!geoTag && profilePage || !geoTag && communityProfilePage) {
        ...
        return (  
            <article className="w-full flex justify-between items-center">
                ...
                {geoTag && <span className={`flex ${profilePage && "flex-row-reverse" || communityProfilePage && "flex-row-reverse"} h-6 text-sm text-gray-500 gap-2 items-center`}>
                {/* {geoTag && (
                    <div>
                        {geoTag.length >= 9 
                        ? (<p className="text-nowrap">{geoTag.slice(0, 9)}</p>) 
